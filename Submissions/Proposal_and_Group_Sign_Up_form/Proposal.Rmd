...
Monte Carlo Simulation of a One-sample Z-test for Proportions of Binomially Distributed Samples.
...
The following outputs will be shown:

- A table showing Type I error rate, Type II error rate and power of the test

- A histogram of simulated p-values, with a vertical line indicating the chosen significance level $\alpha$ [@ggplot22026]. Below is a plot representing what the Shiny output may show.

```{r example_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=1.5, fig.width=3.5, fig.align="center"}
library(ggplot2)
n <- 15
p <- 0.3
n_sim <- 10000
...
Generative AI was used to help format BibTeX references, simulate a ggplot graph for reference, as well as providing tips on using R Markdown and GitHub (e.g. solving merge conflicts). 
